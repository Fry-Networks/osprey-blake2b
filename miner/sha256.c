...
{
    uint32_t a, b, cc, dd, e, f, g, h, i, j, t1, t2, m[64];
...
void sha256_final(sha256_ctx *c, uint8_t out[SHA256_DIGEST_LEN])
{
    ...
    for (i = 0; i < 8; ++i)
        c->data[63 - i] = (uint8_t)(c->bitlen >> (i * 8));
    sha256_transform(c, c->data);

    /* state is big-endian on the wire */
    for (i = 0; i < 4; ++i) {
                 ...
                 uint8_t out[SHA256_DIGEST_LEN])
{
    ...
    sha256((const uint8_t *)tag, strlen(tag), th);

    sha256_init(&c);
    sha256_update(&c, th, sizeof th);   /* tag hash twice, then the payload */
    sha256_update(&c, th, sizeof th);
